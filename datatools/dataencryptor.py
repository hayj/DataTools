...
dataEncryptorSingleton = None
dataEncryptorSingletonLock = Lock()
def getDataEncryptorSingleton(*args, **kwargs):
    global dataEncryptorSingleton
    with dataEncryptorSingletonLock:
        if dataEncryptorSingleton is None:
            dataEncryptorSingleton = DataEncryptor(*args, **kwargs)
...
class DataEncryptor:
    ...
    #         for i in range(10):
    #             if not (isFile(encryptedFilePath) and not isFile(notEncryptedFilePath)):
    #                 sleep(0.1)
    #                 log("Waiting for " + dataname + " to appear...")
            randomName = getRandomStr()
